...
@section('title', "Daftar Telaah Cepat")
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="col-12 mb-3">
                    <h4>{{ !empty($ethic)? $ethic->research->judul: "" }}</h4>
                    <p class="mb-0">Peneliti Utama: {{ !empty($ethic)? $ethic->research->ketua: "" }}</p>
                    @if(!empty($ethic))
                    <a href="{{ route('layaketik.protocol.print', ['hash'=>$hashids->encode($ethic->protocol->id)]) }}" class="btn btn-primary mt-4"><i class="fas fa-print me-2"></i> Print Protokol</a>
                    <button data-id="{{ $ethic->id }}" data-bs-toggle="modal" data-bs-target="#modal_detailresearch" class="btn btn-primary mt-4"><i class="fas fa-info me-2"></i> Lihat Detail</button>
                    @endif
                </div>
            </div>
        </div>
				...
				<div class="row">
                    <div class="col-12">
                        <div class="d-flex justify-content-between mb-4">
                            <h3 class="font-weight-bold">Hasil Telaah Cepat</h3>
    ...
    <script>
        $(document).ready(function(){
            ...
            $.extend(dataProtocols, {
                    "ajax": {
                        "type": "POST",
                        "url": `{{ route('layaketik.telaah.cepat.reviewed', $id) }}`,
                        "timeout": 120000
                    },
                    "aoColumns": [
                        {
                        ...
                        },
                        {
                            ...
                            "render": function (data, row, type, meta) {
                                ...
                                let btn = `
                                <a href="{{ URL::to('/dashboard/layaketik/telaah/cepat/') }}/${ data.id }/1"
